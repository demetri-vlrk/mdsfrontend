...
export function BrandCard() {
  return (
    ...
    >
      <div className="flex h-[87px] w-full items-center justify-center gap-2 rounded-lg bg-bg-subtle px-3 py-[7.5px] shadow-xs">
        <Paperclip className="size-4 shrink-0 text-fg-muted" />
        <span className="text-sm text-fg-default">Drop file here or browse</span>
      </div>
        ...
        {FILES.map(({ name, icon: Icon }) => (
          ...
          >
            ...
            <button
              ...
              className="flex size-4 shrink-0 items-center justify-center text-fg-muted hover:text-fg-default"
            >
              <X className="size-4" />
            </button>
          </div>
