...
function PersonaItem({
  ...
  return (
    <div className="flex flex-1 items-start gap-4 border border-border-subtle p-4">
      <div className="flex flex-1 flex-col gap-1 text-sm">
        <p className="w-full text-fg-default">{title}</p>
        {open && (
          <ul className="list-disc pl-[21px] text-fg-muted">
            {bullets.map((b) => (
              <li key={b}>{b}</li>
            ))}
          </ul>
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="shrink-0 text-fg-default"
      >
...
export function PersonasTensionRow() {
  ...
  return (
    <div className="flex w-full items-start px-8">
      <div className="flex flex-1 flex-col gap-6 border border-border-subtle p-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {PERSONA_TAGS.map((tag) => (
            <span
              key={tag}
              className="flex min-h-8 items-center justify-center rounded-full border border-border-strong bg-white/10 px-3 py-1.5 text-sm font-medium text-fg-default shadow-xs"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex w-full flex-col gap-4">
          <div className="flex w-full items-center gap-3">
            <div className="flex size-10 shrink-0 items-center justify-center rounded-md bg-border-strong">
              <CircleUser className="size-6 text-bg-canvas" />
            </div>
            <div className="flex flex-1 flex-col gap-0.5">
              ...
              <div className="flex items-center gap-2">
                {PERSONA_BADGES.map((b) => (
                  <span
                    key={b}
                    className="flex items-center justify-center rounded-lg border border-border-subtle bg-white/10 px-2 py-0.5 text-xs text-fg-default"
                  >
                    {b}
                  </span>
                ))}
          ...
          <div className="flex w-full items-start gap-4 border border-border-subtle p-4">
            <p className="flex-1 text-sm font-medium text-fg-default">
              Pain points
            </p>
            <button
              type="button"
        ...
        <div className="grid grid-cols-2 gap-y-4">
          {TENSION_STATS.map((stat) => (
            <div key={stat.label} className="flex items-center gap-6 px-6">
              <Package className="size-8 shrink-0 text-fg-muted" />
              <div className="flex flex-col gap-2.5">
                <p className="text-sm font-medium text-fg-muted">
                  {stat.label}
                </p>
                <p className="text-[42px] leading-none font-semibold tracking-[-1.5px] text-fg-default">
                  {stat.value}
                </p>
              </div>
            </div>
