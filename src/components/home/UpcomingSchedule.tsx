...
export function UpcomingSchedule() {
  return (
    <div className="flex h-full flex-1 flex-col items-start gap-6 border border-border-subtle px-8 py-10">
      ...
      <div className="flex w-full flex-col items-start border border-border-subtle">
        {ITEMS.map((item, i) => (
          <div
            key={i}
            className={`flex w-full items-center gap-4 px-6 py-4 ${i < ITEMS.length - 1 ? "border-b border-border-subtle" : ""}`}
          >
            <div className="flex w-12 shrink-0 flex-col items-center rounded-md bg-bg-subtle p-1 text-fg-default">
            ...
            <div className="flex flex-1 flex-col items-start gap-0.5 min-w-0">
              <p className="w-full text-sm font-semibold text-fg-default">
                {item.title}
              </p>
              <p className="w-full text-xs text-fg-muted">{item.time}</p>
            </div>
            <span
